...
plt.ylim(-6,6)
plt.xlim(-6,6)
mp = plt.cm.ScalarMappable(cmap="cool")
mp.set_array([])
bcbar = plt.colorbar(mp)
bcbar.set_ticks([0,1])
...
plt.text(.9,3.5,r'$r_{max}$',rotation=90,color='r',fontsize=14)
plt.title(r"Scattering angle dependence on impact parameter at varying energy")
mp = plt.cm.ScalarMappable(cmap="winter")
mp.set_array([])
ecbar = plt.colorbar(mp)
...
mp = plt.cm.ScalarMappable(cmap="rainbow")
mp.set_array([])
ccbar = plt.colorbar(mp)
